...
class City(Base):
    ...
    rieltorua_id = Column(String(255), nullable=True)
    name = Column(String(255), nullable=False)
    ...
    @classmethod
    def get_active_rieltorua_cities(cls, regiod_id) -> dict:
        response = {}
        with db_session() as session:
            rows = session.query(cls).filter(cls.parsing == True,
                                             cls.region_id == regiod_id,
                                             cls.rieltorua_id.isnot(None)
                                             ).all()
            for row in rows:
                response[row.rieltorua_id] = {'id': row.id,
                                        'name': row.name,
                                        'ai_processed': row.ai_processed,
                                        'region_id': row.region_id,
                                        'detect_photo_duplicates': row.detect_photo_duplicates
                                        }
        return response

    @classmethod
    def get_active_lun_cities(cls) -> dict:
        response = {}
        with db_session() as session:
            rows = session.query(cls).filter(cls.parsing == True,
                                             cls.lun_id.isnot(None)
                                             ).all()
            for row in rows:
                response[row.lun_id] = {"id": row.id,
                                        'ai_processed': row.ai_processed,
                                        'region_id': row.region_id,
                                        'detect_photo_duplicates': row.detect_photo_duplicates
                                        }
...
class Offer(Base):
        ...
        with db_session() as session:
            ...
            for data in new_offers_data:
                if data.get('ad_id') not in uniq_ad_id:
                    ...
                    offer = cls(**data)
                    data.update({'base_price': base_price})
    ...
    @classmethod
    def offers_list_batches(cls, period=30, source='OLX', batch_size=500):
        """
        Генератор, що видає батчі записів напряму з БД (keyset pagination),
        не завантажуючи весь результат у пам'ять і не гідруючи повні ORM-об'єкти.
        """
        period_date = datetime.now() - timedelta(days=period)
        last_id = 0
        ...
        while True:
            ...
            # (ad_id, obj_id, price, is_top, ad_link, currency)
            yield [(r.ad_id, r.id, r.price, r.is_top, r.ad_link, r.currency) for r in rows]
